...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // GSAP owns all animated properties from the start -- no CSS transition fighting it
    gsap.set(nav, {
      xPercent: -50,
      left: '50%',
      top: 0,
    ...
    const handleScroll = () => {
      ...
      if (s === scrolledRef.current) return;
      scrolledRef.current = s;

      gsap.to(nav, {
        top: s ? 20 : 0,
        width: s ? '90%' : '100%',
        maxWidth: s ? '56rem' : '100vw',
        borderRadius: s ? 9999 : 0,
        paddingLeft: s ? '1.5rem' : '5%',
        paddingRight: s ? '1.5rem' : '5%',
        paddingTop: s ? '0.75rem' : '1.25rem',
        paddingBottom: s ? '0.75rem' : '1.25rem',
        duration: 0.8,
        ease: 'power3.inOut',
        overwrite: true,
      });

      // Backdrop fades separately -- opacity is GPU-accelerated, no reflow
      gsap.to(backdrop, {
        opacity: s ? 1 : 0,
        duration: 0.5,
        ease: 'power2.out',
  ...
  return (
    <nav ref={navRef} className="fixed z-50 overflow-hidden">
      <div ref={backdropRef} className="absolute inset-0 bg-[#FAF8F5]/70 backdrop-blur-xl border border-[#C9A84C]/20 rounded-[inherit]" />
      <div className="relative flex items-center justify-between">
        ...
        <button className="bg-accent text-primary btn-pill hover:scale-105 active:scale-95">
          Join Waitlist
        </button>
      </div>
    </nav>
...
const Hero = () => {
  ...
  return (
    <section ref={containerRef} className="relative pt-32 pb-20 px-8 max-w-[1400px] mx-auto overflow-hidden">
      <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-12 items-start">
        {/* Hero Copy */}
        <div className="hero-copy flex flex-col pt-12">
          <div className="hero-reveal text-accent font-mono text-[10px] uppercase tracking-[0.4em] mb-8">The premier cigar app</div>
          <h1 className="hero-reveal text-primary font-bold text-5xl md:text-7xl lg:text-8xl tracking-tighter leading-[0.9] mb-8">
            For the smoker whose standards keep getting better.
          </h1>
          <div className="hero-reveal flex items-center gap-4 mb-8">
            <div className="w-12 h-px bg-accent/30" />
            <div className="text-primary/40 font-medium text-xs uppercase tracking-widest">Collector memory, private access, lounge-grade taste</div>
          </div>
          <p className="hero-reveal text-primary/60 text-lg md:text-xl leading-relaxed max-w-xl mb-12">
            Inked Draw turns cigar discovery, tasting memory, and private access into one composed membership experience. It feels less like a tracker and more like a modern lounge.
          </p>
          <div className="hero-reveal flex gap-4 flex-wrap mb-24">
            <button className="bg-primary text-background btn-premium">
              <span className="relative z-10">Request private access</span>
            </button>
            <button className="border border-primary/10 text-primary btn-premium hover:bg-primary/5">
              <span className="relative z-10">See the experience</span>
            </button>
          </div>

          {/* Trust Items */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 pt-12 border-t border-primary/5">
            {[
              { label: "Collector memory", title: "Notes with taste.", desc: "Capture draw, body, finish, and mood without the admin." },
              { label: "Private access", title: "Chosen, not pushed.", desc: "Invitation-first framing for a more selective posture." },
              { label: "Commercial clarity", title: "Easy to want.", desc: "The offer reads faster because messages have a place to land." }
            ].map((item, i) => (
              <div key={i} className="hero-reveal">
                <div className="text-[10px] text-accent font-bold uppercase tracking-widest mb-2">{item.label}</div>
                <h3 className="text-sm font-bold mb-2">{item.title}</h3>
                <p className="text-xs text-primary/50 leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="hero-visual-reveal relative">
          <div className="relative rounded-premium overflow-hidden aspect-[4/5] bg-primary group shadow-2xl">
            <img 
              src="/idhero2.png" 
              alt="Inked Draw Atmosphere" 
            ...
            <div className="absolute bottom-10 left-10 right-10">
              <div className="text-accent font-mono text-[10px] uppercase tracking-widest mb-2">Collector atmosphere</div>
              <h2 className="text-background text-3xl font-bold tracking-tight mb-4 leading-tight">Warm wood, dark glass, one confident focal point.</h2>
              <p className="text-background/50 text-sm max-w-xs">Everything above the fold now reads with less sales pressure and more private assurance.</p>
            </div>

            {/* Device Shell (Mockup) */}
            <div className="absolute -right-20 top-20 w-[280px] drop-shadow-[0_40px_80px_rgba(0,0,0,0.5)] rotate-6 group-hover:rotate-3 transition-transform duration-1000 ease-out">
              <img src="/inked-phone.png" alt="Inked App Interface" className="w-full h-auto" />
            </div>
          </div>

          {/* Support Panels */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
            <div className="p-8 rounded-premium bg-white border border-primary/5 shadow-xl">
              <div className="text-[10px] text-accent font-bold uppercase tracking-widest mb-4">For the host</div>
              <h3 className="text-lg font-bold mb-2 tracking-tight">Guide the evening with better recall.</h3>
              <p className="text-xs text-primary/60 leading-relaxed">Remember what landed, what paired beautifully, and what deserves another place in the humidor.</p>
            </div>
            <div className="p-8 rounded-premium bg-primary text-background border border-white/5 shadow-xl">
              <div className="text-[10px] text-accent font-bold uppercase tracking-widest mb-4">For the brand</div>
              <h3 className="text-lg font-bold mb-2 tracking-tight">Sharper, quieter, more spoken for.</h3>
              <p className="text-background/60 text-xs leading-relaxed">The finish feels more editorial now, with less obvious premium signaling and more actual confidence.</p>
            </div>
    </section>
  );
};
